...
- **Base URL**: Base URL: Actually, this app can be run locally and it is hosted also as a base URL using heroku (the heroku URL is https://capstone-project-v1.herokuapp.com/). The backend app is hosted at the default, `http://127.0.0.1:5000/`, which is set as a proxy in the frontend configuration.
...
- Require the `get:shows` permission
...
- Require the `post:shows` permission
- Create a new row in the shows table
- Contain the actor_id and movie_id data representation
returns status code 200 and json `{"success": True, "new show":new shows` where actor an array containing only the newly created actor or appropriate status code indicating reason for failure
